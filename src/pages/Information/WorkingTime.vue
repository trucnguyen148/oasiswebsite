<template>
  <div v-if="!$apolloData.queries.workTimes.loading" >
    <md-card>
              ...
              <div class="md-layout">
                ...
                <div class="md-layout-item md-size-100">
                  <md-field>
                    ...
                    <md-input v-model="addWorkTimeParams.start" type="time" required></md-input>
                  </md-field>

                  <md-field>
                    <label>End Time</label>
                    <md-input v-model="addWorkTimeParams.end" type="time" required></md-input>
                  </md-field>
                </div>
              </div>
            </sui-modal-content>
            <sui-modal-actions>
              <sui-button positive @click.native="addWorkTime">Add</sui-button>
            </sui-modal-actions>
          </sui-modal>
        </div>
              ...
              <md-table md-card>
                <md-table-row>
                  <md-table-cell md-label="Picture">
                    <li style="margin: 0 2rem">{{workTime.start}} - {{workTime.end}}</li>
                  </md-table-cell>
          ...
          <sui-modal-content image>
            <div class="md-layout">
              ...
              <div class="md-layout-item md-size-100">
                <md-field>
                  <label>Start Time</label>
                  <md-input v-model="updateWorkTimeParams.start" type="time" required></md-input>
                </md-field>

                <md-field>
                  <label>End Time</label>
                  <md-input v-model="updateWorkTimeParams.end" type="time" required></md-input>
                </md-field>
              </div>
            </div>
          </sui-modal-content>
...
const ADD_WORKTIME = gql`
  mutation($start: String!, $end: String!) {
    createWorkTime(input: { start: $start, end: $end }) {
      id
      start
      end
    }
...
const UPDATE_WORKTIME = gql`
  mutation($id: ID!, $start: String!, $end: String!) {
    updateWorkTime(id: $id, input: { start: $start, end: $end }) {
      id
      start
      end
    }
