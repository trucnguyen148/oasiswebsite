  ...
  <div v-if="!$apolloData.queries.positions.loading" id="pricelist">
    <md-card>
      ...
      <md-card-content>
          ...
          <sui-modal v-model="open">
            ...
            <sui-modal-content>
              ...
              <div class="md-layout-item md-size-100">
                <md-field>
                  <label>Name</label>
                  <md-input v-model="addCategoryPrams.name" type="text" required></md-input>
                </md-field>
              </div>
            </sui-modal-content>
            <sui-modal-actions>
        ...
        <ul v-for="category in $apolloData.data.positions" v-bind:key="category.id">
          <div class="setting">
            <li style="margin: 0 2rem">
              <label>{{category.name}}</label>
            </li>
            <sui-button @click.native="toggleService(category.id)">
              <font-awesome-icon icon="plus-circle" />
            </sui-button>
          </div>
            ...
            <sui-modal-content image>
              <div class="md-layout">
                ...
                <div class="md-layout-item md-smal-size-100 md-size-50">
                  <md-field>
                    <label>Name</label>
                    <md-input v-model="addServicePrams.name" type="text" required></md-input>
                  </md-field>
                </div>
                <!-- Descriptions -->
                <div class="md-layout-item md-size-100">
                ...
                <div class="md-layout-item md-smal-size-100 md-size-50">
                  <md-field>
                    <label>Price</label>
                    <md-input v-model="addServicePrams.unit_price" type="number" required></md-input>
                  </md-field>
                </div>
              </div>
            </sui-modal-content>
          ...
          <div class="md-layout-item md-size-100">
            <md-field>
              <md-table md-card>
                <md-table-row v-for="product in category.products" v-bind:key="product.id">
          ...
          <sui-modal-content image>
            <div class="md-layout">
              ...
              <div class="md-layout-item md-smal-size-100 md-size-50">
                <md-field>
                  <label>Name</label>
                  <md-input v-model="updateServicePrams.name" type="text" required></md-input>
                </md-field>
              </div>
              <!-- Descriptions -->
              <div class="md-layout-item md-size-100">
              ...
              <div class="md-layout-item md-smal-size-100 md-size-50">
                <md-field>
                  <label>Price</label>
                  <md-input v-model="updateServicePrams.unit_price" type="text" required></md-input>
                </md-field>
              </div>
            </div>
          </sui-modal-content>
          <sui-modal-actions>
            <sui-button positive @click.native="updateService()">Update</sui-button>
          </sui-modal-actions>
        </sui-modal>
      </md-card-content>
    </md-card>
...
const ADD_CATEGORY = gql`
  mutation($type: Int!, $name: String!) {
    createCategory(input: { type: $type, name: $name }) {
      id
      name
    }
  }
...
const UPDATE_SERVICE = gql`
  mutation($id: ID!, $name: String!, $description: String!, $unit_price: Int!) {
    updateProduct(
      id: $id
      input: { name: $name, description: $description, unit_price: $unit_price }
    ) {
      id
    }
  }
...
export default {
  ...
  methods: {
    ...
    addCategory() {
      ...
      const description = this.addCategoryPrams.description;
